...
switch($_GET["action"]) {
	case "add":
		if(!empty($_POST["quantity"])) {
			$productByProductId = $db_handle->runQuery("SELECT * FROM my_products WHERE ProductId='" . $_GET["ProductId"] . "'");
			$itemArray = array($productByProductId[0]["ProductId"]=>array('name'=>$productByProductId[0]["name"], 'ProductId'=>$productByProductId[0]["ProductId"], 'quantity'=>$_POST["quantity"], 'price'=>$productByProductId[0]["price"]));
...
<?php		
    foreach ($_SESSION["cart_item"] as $item){
		?>
				...
				<td style="text-align:center;border-bottom:#F0F0F0 1px solid;"><a href="index.php?action=remove&ProductId=<?php echo $item["ProductId"]; ?>" class="btnRemoveAction">Remove Item</a></td>
				</tr>
				<?php
